...
HUB_ID="${HF_NAMESPACE}/${MODEL_ALIAS}_smcalflow-baseline-2epoch"
if model_exists "$HUB_ID"; then
    echo "SKIP $HUB_ID (exists)"
else
    uv run python src/train.py \
        ...
        --train_path "$TRAIN_PATH" \
        --valid_path "$VALID_PATH" \
        --output_dir "outputs/${MODEL_ALIAS}-lora-smcalflow-baseline-2epoch" \
        --hub_model_id "$HUB_ID"
fi

HUB_ID="${HF_NAMESPACE}/${MODEL_ALIAS}_smcalflow-mixed"
if model_exists "$HUB_ID"; then
    echo "SKIP $HUB_ID (exists)"
else
    uv run python src/train.py \
