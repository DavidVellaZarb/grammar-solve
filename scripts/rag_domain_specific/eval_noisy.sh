...
RESULT_DIR=results/rag_domain_specific/noisy
...
uv run python src/plot.py plot_multi_metrics \
    --result_files '["results/smiles/baseline/test.json", "'"${RESULT_DIR}"'/smiles/test.json", "results/ablations/smiles_add_remove_rule_p20/gold_grammar.json"]' \
    ...
    --title "SMILES — RAG Grammar Prediction (Noisy Training Grammars)"
...
uv run python src/plot.py plot_multi_metrics \
    --result_files '["results/spice/baseline/test.json", "'"${RESULT_DIR}"'/spice/test.json", "results/ablations/spice_add_remove_rule_p20/gold_grammar.json"]' \
    ...
    --title "SPICE — RAG Grammar Prediction (Noisy Training Grammars)"
...
uv run python src/plot.py plot_multi_metrics \
    --result_files '["results/openscad/baseline/test.json", "'"${RESULT_DIR}"'/openscad/test.json", "results/ablations/openscad_add_remove_rule_p20/gold_grammar.json"]' \
    ...
    --title "OpenSCAD — RAG Grammar Prediction (Noisy Training Grammars)"
...
uv run python src/plot.py plot_pass_at_k \
    --result_files '["results/verilog/baseline.json", "'"${RESULT_DIR}"'/verilog/test.json", "results/ablations/verilog_add_remove_rule_p20/gold_grammar.json"]' \
    --labels '["Baseline", "RAG", "Gold Grammar"]' \
    --output_path "${RESULT_DIR}/verilog/pass_at_k.png" \
    --title "Verilog — RAG Grammar Prediction (Noisy Training Grammars)"
