...
HUB_ID="${HF_NAMESPACE}/${MODEL_ALIAS}_geoquery-baseline-2epoch"
if model_exists "$HUB_ID"; then
    echo "SKIP $HUB_ID (exists)"
else
    uv run python src/train.py \
        ...
        --train_path "$TRAIN_PATH" \
        --valid_path "$VALID_PATH" \
        --output_dir "outputs/${MODEL_ALIAS}-lora-geoquery-baseline-2epoch" \
        --hub_model_id "$HUB_ID"
fi

HUB_ID="${HF_NAMESPACE}/${MODEL_ALIAS}_geoquery-mixed"
if model_exists "$HUB_ID"; then
    echo "SKIP $HUB_ID (exists)"
else
    uv run python src/train.py \
        ...
        --train_path "$TRAIN_PATH" \
        --valid_path "$VALID_PATH" \
        --output_dir "outputs/${MODEL_ALIAS}-lora-geoquery-mixed" \
        --hub_model_id "$HUB_ID"
fi
