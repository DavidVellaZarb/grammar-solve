...
SYSTEM_PROMPT_TEMPLATES: dict[str, str] = {
    ...
    "spice": (
        ...
        "- Your grammar should have exactly ONE alternative per physical component instance.\n"
        "  Do NOT add components that are inferred or \"typical\" but not stated in the description\n"
        ...
        "  but do NOT copy their component counts or values for a different circuit\n"
        ...
        "First inventory the components from the description, then think step-by-step about\n"
        "what components are needed and how they connect, then output the grammar inside\n"
        "<grammar>...</grammar> tags.\n\n"
    ...
    "spice_cot": (
        ...
        "  resistor ::= COMP_R node node value?\n"
        "  voltage_source ::= COMP_V node node source_spec*\n\n"
        "COMPONENT INVENTORY (do this BEFORE writing the grammar):\n"
        "List the components explicitly mentioned in the description and count each type\n"
        "(resistors, capacitors, voltage sources, transistors, etc.). Your grammar should\n"
        "have exactly ONE alternative per physical component instance. Do NOT add components\n"
        "that are inferred or \"typical\" but not stated in the description.\n\n"
        "IMPORTANT: fewer wrong rules is better than broader coverage. If you are unsure\n"
        "whether a component exists in the circuit, leave it out. The downstream model will\n"
        "produce every component listed in the grammar, so every spurious alternative directly\n"
        "hurts output quality.\n\n"
        ...
        "  commands are available, NOT as an output format template\n"
        "- Include only the rules needed for this specific circuit\n\n"
        "Your task: first inventory the components from the description, then reason\n"
        "step-by-step about which grammar rules are needed for this circuit (and why),\n"
        "then predict the minimal grammar.\n\n"
        "Follow the same reasoning pattern shown in the examples: explain why each rule\n"
